<script lang="ts">
  ...
  onMount(() => {
    if (
      props.dataForm.data.image === '' ||
      props.dataForm.data.image === null
    ) {
      imageUrl =
        'https://res.cloudinary.com/dcew4hgxp/image/upload/v1714012419/dummy-image.svg';
    } else {
    ...
    <div class="flex flex-col w-full gap-2">
      <div class="w-full flex flex-col gap-1 mb-4">
        ...
        {#if edit || props.isCreated === true}
          <span class="pb-2 pl-0 text-xl text-[#4c4c4c] font-semibold">
            Product Gallery
          </span>
          <div
      ...
      <div
        class="flex flex-col lg:flex-row items-center justify-end w-full gap-2"
      >
        {#if props.isCreated}
          <form
