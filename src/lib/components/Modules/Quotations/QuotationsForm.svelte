<script lang="ts">
	...
	export let props = {
		...
		products: { value: string; label: string; price: number; image: string; stock: number }[];
	...
	onMount(() => {
		...
		if (props.isEditing) {
			readonly = true;
			imageUrl = props.products.find((product) => product.value === $form.product_id.value)?.image;
		}
	});
</script>
...
<section class="bg-white rounded-2xl mb-4">
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-7">
		<div class="flex flex-col gap-3">
			<span class="pb-1 pl-0 text-xl text-[#4c4c4c] font-semibold block lg:hidden"
				>Product Preview
			</span>
			{#if imageUrl}
				<img
			...
			{:else}
				...
				<Select
					...
					on:change={(e) => {
						...
						imageUrl = props.products.find((product) => product.value === e.detail.value)?.image;
					}}
				/>
			{/if}
			<Input
		...
		<div class="w-full flex flex-col gap-3">
			<span class="pb-1 pl-0 text-xl text-[#4c4c4c] font-semibold hidden lg:block"
				>Product Preview
			</span>
			{#if imageUrl}
				<img
			...
			<div class="flex flex-col lg:flex-row items-center justify-end w-full gap-4">
				{#if props.isCreated}
					<form action="?/createQuotation" method="POST" use:enhance on:submit class="w-full">
