...
export const load = async ({ fetch, params }) => {
  const getProductById = async (_id: string) => {
    try {
      const { statusCode, data, message } = await fetch(
        `${PUBLIC_APP_URL}/api/products/${_id}`,
        {
          method: 'GET',
  ...
  return {
    form: await superValidate(
      await getProductById(params._id),
      yup(ProductSchema)
    ),
    formRemove: await superValidate(
      { id: params._id },
      yup(DeleteProductSchema)
    ),
  };
};
  ...
  updateProduct: async (event) => {
    try {
      ...
      const { statusCode, message } = await fetch(
        `${PUBLIC_APP_URL}/api/products/${form.data.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
