...
export const load: PageServerLoad = async ({ params }) => {
  ...
  const getQuotationById = async (_id: string) => {
    try {
      const { statusCode, data, message } = await fetch(
        `${PUBLIC_APP_URL}/api/quotations/${_id}`,
        {
          method: 'GET',
      ...
      if (statusCode !== 200)
        throw error(statusCode, `Error al cargar la cotización: ${message}`);
  ...
  return {
    ...
    form: await superValidate(
      await getQuotationById(params._id),
      yup(QuotationSchema)
    ),
  };
};
